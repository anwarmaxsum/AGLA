            ...
            for oe in range(self.outer_epochs):
                ...
                for ie in range(self.inner_epochs):
                    ...
                    for images, targets in trf_loader:
                        ...
                        if(t>0):
                            curr_trans_mem_outputs = self.model(self.trans_mem_x)
                            ...
                            loss = self.criterion(t, outputs, targets.to(self.device).type(torch.long))

                        self.assessorOptimizer.zero_grad()
                        loss.backward()
                        self.assessorOptimizer.step()
                    print('I-Epoch :{:3d} {:3d} {:3d} loss:{:5.1f} {:5.1f} {:5.1f}'.format(e+1,oe+1, oe+1,loss, loss1, loss2))            
                ...
                for images, targets in trn_comb_loader:
                    ...
                    if(t>0):
                        ...
                        loss = (c_lr[0] * loss1)+(c_lr[1] * loss2 * t)
                    else:
                        loss = c_lr[0] * self.criterion(t, outputs, targets.to(self.device))
                ...
                clock2 = time.time()
                print('| O-Epoch {:3d} {:3d}, time={:5.1f}s/{:5.1f}s | c_lr={:.3f} {:.3f} |Valid: loss={:.3f} {:.3f} {:.3f} {:.3f}, TAw acc={:5.1f}% |'.format(
    ...
    def generate_transformed_data(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            if(notSaved):
                ...
                self.save_aug_image(x1,"3xtrf_invert.jpg")
                notSaved=False
