...
from torchvision.datasets import Caltech101 as TorchVisionCaltech101
...
def get_datasets(dataset, path, num_tasks, nc_first_task, validation, trn_transform, tst_transform, class_order=None):
    ...
    if 'mnist' in dataset:
        ...
        # compute splits
        print("===============")
        print(tvcifar_trn.data[0])
        print("-------------------")
        print(tvcifar_trn.targets[0])
        print("===============")
        all_data, taskcla, class_indices = memd.get_data(trn_data, tst_data, validation=validation,
                                                         num_tasks=num_tasks, nc_first_task=nc_first_task,
                                                         shuffle_classes=class_order is None, class_order=class_order)
        ...
        #print(path)
        #caltech101dataset = TorchVisionCaltech101(path, download=True)
        #(X, x_val , Y, y_val) = train_test_split(caltech101dataset, caltech101dataset,
        #                                            test_size=0.2,
        #                                            random_state=42)
        #print("size of caltech101 " + np.dim(caltech101dataset))
        #tvcaltech101_trn = TorchVisionCaltech101(path, train=True, download=True)
        #tvcaltech101_tst = TorchVisionCaltech101(path, train=False, download=True)
        hf = h5py.File(path+'/caltech101_224x224.h5', 'r')
        trainX = np.array(hf.get('trainX'))
        trainY = np.array(hf.get('trainY'))
        testX = np.array(hf.get('testX'))
        testY = np.array(hf.get('testY'))
        trn_data = {'x': trainX, 'y': trainY}
        tst_data = {'x': testX, 'y': testY}
        # compute splits
        all_data, taskcla, class_indices = memd.get_data(trn_data, tst_data, validation=validation,
                                                         num_tasks=num_tasks, nc_first_task=nc_first_task,
                                                         shuffle_classes=class_order is None, class_order=class_order)
        ...
        hftr = h5py.File(path+'/cub200_train_224x224.h5', 'r')
        hfts = h5py.File(path+'/cub200_test_224x224.h5', 'r')
        #hftr = h5py.File(path+'/cub200_224x224.h5', 'r')
        trainX = np.array(hftr.get('trainX'))
        trainY = np.array(hftr.get('trainY'))
        testX = np.array(hfts.get('testX'))
        testY = np.array(hfts.get('testY'))
        trn_data = {'x': trainX, 'y': trainY}
        tst_data = {'x': testX, 'y': testY}
        # compute splits
        all_data, taskcla, class_indices = memd.get_data(trn_data, tst_data, validation=validation,
        ...
        hf = h5py.File(path+'/miniimagenet_train_test_32x32.h5', 'r')
        trainX = np.array(hf.get('trainX'))
        trainY = np.array(hf.get('trainY'))
        testX = np.array(hf.get('testX'))
        testY = np.array(hf.get('testY'))
